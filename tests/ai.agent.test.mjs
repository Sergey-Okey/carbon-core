...
test('AI endpoint stays server-side, rate limited, and defaults to OpenRouter', async () => {
  ...
  assert.match(endpoint, /resolveAiBaseUrl/)
  assert.match(endpoint, /process\.env\.NUXT_AI_ENGINE \|\| process\.env\.AI_ENGINE/)
  assert.doesNotMatch(endpoint, /AI is not configured/)
  assert.doesNotMatch(endpoint, /Authentication required/)
  assert.match(config, /aiEngine: process\.env\.AI_ENGINE \|\| 'openai'/)
  assert.match(config, /OPENROUTER_PROXY_URL/)
  assert.match(config, /openrouter\.ai\/api\/v1/)
  assert.match(config, /process\.env\.OPENAI_API_KEY/)
  assert.match(llm, /resolveAiApiKey/)
  assert.match(llm, /resolveAiBaseUrl/)
  assert.match(llm, /OPENROUTER_PROXY_URL/)
  assert.match(llm, /NUXT_OPENAI_API_KEY/)
  assert.match(llm, /resolveOpenRouterProxySecret/)
  assert.match(llm, /x-cof-proxy-secret/)
  ...
  const proxyWorker = await read('deploy/openrouter-proxy-worker.js')
  const proxyToml = await read('deploy/wrangler.openrouter-proxy.toml')
  assert.match(proxyWorker, /openrouter\.ai/)
  assert.match(proxyWorker, /x-cof-proxy-secret/)
  assert.match(proxyWorker, /\/api\/v1\/chat\/completions/)
  assert.doesNotMatch(proxyWorker, /x-forwarded-for/)
  assert.match(proxyToml, /cof-openrouter-proxy/)
  assert.match(pollinations, /text\.pollinations\.ai/)
  assert.match(pollinations, /openai-fast/)
  assert.match(pollinations, /method: 'GET'/)
  assert.match(pollinations, /origin/)
  assert.match(pollinations, /referer/)
  assert.match(pollinations, /execFile/)
  assert.match(pollinations, /curlAsResponse/)
  assert.doesNotMatch(pollinations, /jsonMode/)
