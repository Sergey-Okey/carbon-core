...
test('AI endpoint stays server-side, rate limited, and defaults to OpenRouter', async () => {
  ...
  assert.match(layout, /launchReady/)
  ...
  assert.match(index, /LazyBranchMobileView v-if="isMobileBoard"|BranchMobileView v-if="isMobileBoard"/)
  ...
  assert.match(headerActions, /demo-access/)
  assert.doesNotMatch(headerActions, /HelpCircle|Открыть обучение/)
  assert.match(launch, /launch-ready/)
