...
test('nested radius tokens and mixins stay concentric', async () => {
  ...
  assert.match(mixins, /@mixin island-shell/)
  assert.match(mixins, /@mixin island-control/)
  assert.match(
    mixins,
    /@mixin nest-shell\([\s\S]*?--radius-nested:\s*max\(0px,\s*calc\(var\(--nest-radius\) - var\(--nest-pad\)\)\)/
  )
  assert.match(mixins, /@mixin island-shell \{[\s\S]*?@include nest-shell\(var\(--radius-full\)/)
...
test('spacing and typography tokens keep a 4px scale and heading roles', async () => {
  ...
  assert.match(spacing, /--island-item:\s*var\(--space-12\)/)
  assert.match(spacing, /--board-island-width:/)
